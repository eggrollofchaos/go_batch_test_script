# COMS 4113: Batch Test Runner (`run_tests.sh`) - v1.07.2
...
### UPDATE: V1.07.2 - Added explicit TRACE log level
Sometimes you need more logging, and making them all Debug is confusing and lacks proper control. See instructions below for how to define and use TRACE custom log level via `slog`.
Additionally, added instructions and examples for using _infix string format_ for variables in log messaging.
...
### 2. Logging Setup via `slog`
This is required for using `v` verbosity command line flags.

Presumably you are already using `slog` for logging because of how simple it is to set log level at runtime. If so, you may have already set up command line flags for this purpose. If not, here is a simple implementation that the author uses.  
...
#### `slog` command line flag parsing
...
func TestMain(m *testing.M) {
  ...
  logLevelFlag := flag.Int("loglevel", 0, "set debug log level (default 0: None, 1=Error, 2=Warn, 3=Info, 4=Debug, 5=Trace")
  flag.Parse()
  ...
  // Only add case 5 if you will be using TRACE log level
  case 5:
    logLevel = LevelTrace   // note the difference here, due to custom log level
  }
...
#### Defining TRACE logging
For using the 5th log level (Trace), add this code to your source `.go` file:

```go
// Define custom Trace log level, define LogTrace function
const (
  LevelTrace = slog.Level(-8) // for logging more verbose than Debug
)
...
#### Using infix string format
If desired, additional logging can be defined to use _infix string formatting_ for variable display, instead of the default key-value pairs using structs. To use, simply add this to your source `.go` file:
```go
// Define LogTraceString function
func LogTraceString(format string, args ...any) {
  msg := fmt.Sprintf(format, args...)
  slog.Log(context.Background(), LevelTrace, msg)
}
```
Note that this format can be defined for any log level (Error / Warn / Info / Debug / Trace), see examples below.

#### Sample log messages
Add directly in source `.go` files:
```go
// default key-value pairs, variables displayed as structs
slog.Error("LOG - kvpaxos | server.Get() - op not found in cache after success")
slog.Warn("LOG - shardkv | tick() - GetShard RPC timed out",
  "srv", srv)
slog.Info("LOG - base | state.RandomNextState() - no events possible, returning current state.")
slog.Debug("LOG - paxos | server.NewServer() - func called:",
  "peers", peers, "me", me, "proposedValue", proposedValue)
LogTrace("LOG - shardmaster | client.call() - attempting RPC:",
  "srv", srv, "rpcname", rpcname, "args", args, "reply", reply)

// infix string format
LogWarnString("LOG - paxos | server.MessageHandler() - ProposeResponse: server[%d] duplicate or unknown peer response",
  server.me)
LogDebugString("LOG - paxos | server.MessageHandler() - ProposeResponse: server[%d] updated V to %v from peer with higher N_a of %d",
  server.me, msg.V_a, msg.N_a)
LogTraceString("LOG - paxos | server.MessageHandler() - ProposeRequest: server[%d] handle ProposeRequest: From=%s, N=%d (my n_p=%d)", 
  server.me, msg.From(), msg.N, node.n_p)
```
...
> ```bash
> $ go test -loglevel 1
> {"time":"2025-11-16T13:55:48.92208-05:00","level":"ERROR","msg":"LOG - kvpaxos | server.Get() - op not found in cache after success"}
> Test: Basic leave/join ...
> ...
> ```

Everything `Trace` and below, `-vvvvv` in test script:  
(TRACE example here uses infix string formatting)
> ```bash
> $ go test -loglevel 5
> {"time":"2025-11-16T13:46:51.627065-05:00","level":"ERROR","msg":"LOG - kvpaxos | server.Get() - op not found in cache after success"}
> {"time":"2025-11-16T13:46:51.627174-05:00","level":"WARN","msg":"LOG - shardkv | tick() - GetShard RPC timed out","srv":"/var/tmp/824-501/skv-60629-moves-0"}
> {"time":"2025-11-16T13:46:51.627176-05:00","level":"INFO","msg":"LOG - base | state.RandomNextState() - no events possible, returning current state."}
> {"time":"2025-11-16T13:46:51.627177-05:00","level":"DEBUG","msg":"LOG - paxos | server.NewServer() - func called:","peers":["s1","s2","s3"],"me":0,"proposedValue":"v1"}
> {"time":"2025-11-16T13:46:51.627180-05:00","level":"DEBUG-4","msg":"LOG - paxos | server.MessageHandler() - ProposeRequest: server[0] handle ProposeRequest: From=s3, N=2 (my n_p=2)"}
> Test: Basic leave/join ...
> ...
> ```

Etc.  
Note that during normal use of this batch test script, the above log lines would not appear in terminal output; they are captured and redirected to log files.
...
- **For A5 (paxos model checker):** `cd pkg` (or the root directory with `go.mod`)  
  Note: For Assignment 5 parts A/B/C, dependent upon environment setup, you may need to run `run_test.sh` from within each subdirectory.
...
### 3. Execution examples (advanced)

```bash
# (Serial) run all tests 100 times, no runtime logging, 1m/2m slow/timeout
./run_tests.sh

# (Serial) run TestUnreliable 50 times, loglevel Error, 45s/2m slow/timeout
./run_tests.sh -v 50 TestUnreliable -s 45s

# (Parallel) run all tests in directory 500 times, 3 processes per test, progress report every 30 tests, loglevel Warn, 30s/1m slow/timeout
./run_tests.sh -pvv -n 3 -g 30 -s 30s -t 1m

# (Parallel) run TestBasic 500 times, 20 processes per test, progress report every 10 tests, loglevel Info, 30s/2m slow/timeout
./run_tests.sh -pvvv -n 20 -s 30s TestBasic

# (Parallel) run test suite A4A 1000 times, 2 processes per test, progress report every 20 tests, loglevel Debug, 1m/90s slow/timeout
./run_tests.sh -pvvvv -g 20 -z A4A -t 90s 1000

# (Parallel) run all tests in directory 100 times, 2 processes per test, progress report every 1 tests, loglevel Trace, 10s/30s slow/timeout
./run_tests.sh -pvvvvv -g 1 -s 10s -t 30s 100
```
...
4. **File artifacts:**
    - `output_...log`: Detailed logs for any run that was **Failed** or **Slow**.
      - Logs for passing tests are deleted to conserve disk space, especially important for large parallel batch testing jobs set to a high log verbosity level (we're talking gigabytes).
