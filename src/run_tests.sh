...
# Changes: https://github.com/eggrollofchaos/go_batch_test_script/CHANGELOG.md

# CHANGES IN VERSION 1.07.2
# Added explicit TRACE log level
...
VERBOSE=0                           # runtime logging verbosity level: 0=None, 1=Error, 2=Warn, 3=Info, 4=Debug, 5=Trace
VERBOSE_STR="None"
VERBOSE_CMD=""
...
while getopts "pn:g:z:vs:t:h" opt; do
  ...
  v )                                       # set verbosity level
    ...
    # Cap at max verbosity level (5)
    if (( VERBOSE > 5 )); then
      VERBOSE=5
      echo "Note: Maximum verbosity level (5=Trace) reached. Additional -v flags ignored."
    fi
    case $VERBOSE in
    1)
      VERBOSE_STR="Error"
  ...
  h )                                       # set to show this Help
    ...
    echo "  -v[v][v][v][v]  Set runtime logging verbosity (-v Error, -vv Warning, -vvv Info, -vvvv Debug, -vvvvv Trace). Default is none, however system level logs from <go test> will always print."
    ...
    echo
    echo "Examples:"
    echo "  $0                                   # (Serial) run all tests 100 times, no runtime logging, 1m/2m slow/timeout."
    echo "  $0 -v 50 TestUnreliable -s 45s       # (Serial) run TestUnreliable 50 times, loglevel Error, 45s/2m slow/timeout."
    echo "  $0 -pvv -n 3 -g 30 -s 30s -t 1m      # (Parallel) run all tests in directory 500 times, 3 processes per test,"
    echo "                                           progress report every 30 tests, loglevel Warn, 30s/1m slow/timeout."
    ...
    echo "  $0 -pvvvv -g 20 -z A4A -t 90s 1000   # (Parallel) run test suite A4A 1000 times, 2 processes per test,"
    echo "                                           progress report every 20 tests, loglevel Debug, 1m/90s slow/timeout."
    echo "  $0 -pvvvvv -g 1 -s 10s -t 30s 100    # (Parallel) run all tests in directory 100 times, 2 processes per test,"
    echo "                                           progress report every 1 tests, loglevel Trace, 10s/30s slow/timeout."
    echo
    exit 0
    ;;
  \? )
  ...
  (                 # Uses a lock mechanism not dissimilar to what we learned in class
    ...
    if (( not_run > 0 )); then
      ...
      # If a test chunk has a mix of SLOW vs FAILED results, FAILED will take precedence
        ...
        # Delete log file -- to save space! especially when doing DEBUG or TRACE logging
        rm "$outfile"
      fi
    fi
